...
export class VideoRTC extends HTMLElement {
    constructor() {
         ...
         * @type {string}
         */
        this.state = 'webrtc,mse,hls,mjpeg'
    ...
    onconnect() {
        ...
        console.log(new WebSocket(this.wsURL), this.wsURL)
    ...
    onopen() {
        ...
        if (
            this.state.indexOf('mse') >= 0 &&
            ('MediaSource' in window || 'ManagedMediaSource' in window)
        ) {
            modes.push('mse')
            this.onmse()
        } else if (
            this.state.indexOf('hls') >= 0 &&
            this.video.canPlayType('application/vnd.apple.mpegurl')
        ) {
            modes.push('hls')
            this.onhls()
        } else if (this.state.indexOf('mp4') >= 0) {
            modes.push('mp4')
            this.onmp4()
        }

        if (
            this.state.indexOf('webrtc') >= 0 &&
    ...
    onwebrtc() {
        ...
        pc.addEventListener('icecandidate', (ev) => {
            if (
                ev.candidate &&
                this.state.indexOf('webrtc/tcp') >= 0 &&
